...
def gemini_embed_query(text: str) -> List[float]:
    """Embed a query for screen activity search, matching whatever embedding
    space generate_embedding()/RETRIEVAL_DOCUMENT indexing already uses.

    Previously called Google's generativelanguage API directly (with an empty
    API key falling through as a real, unauthenticated request) even when no
    Gemini key or gateway route was configured — a real self-hosted
    deployment has neither, so this now always routes through the same local
    embeddings client (OPENAI_BASE_URL) that the rest of the RAG pipeline
    uses, instead of ever reaching Google.
    """
    ...
    if byok_key or os.environ.get('GEMINI_API_KEY', ''):
        api_key = byok_key or os.environ['GEMINI_API_KEY']
